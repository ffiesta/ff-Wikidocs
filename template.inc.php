...
<head>
  ...
  <link type="text/css" rel="stylesheet" href="<?= $APP->PATH ?>helpers/katex-0.16.10/katex.min.css" media="screen,projection">
...
<main>
  ...
  <div class="container">
      ...
      <div class="col s2 m2 l2">
        <?php if(MODE=="view"): ?>
          <span class="right nowrap">
            ...
            <?php if(Session::getInstance()->autenticationLevel()==2): ?>
              ...
              <?php if($DOC->VERSION!=="latest"): ?><form method="post" action="<?= $APP->PATH ?>submit.php?act=content_restore" style="display:inline;" onsubmit="return(confirm('<?= str_replace(["'",'"'],"\'",$TXT->TooltipRestoreVersionConfirm) ?>'))"><input type="hidden" name="token" value="<?= htmlspecialchars(Session::getInstance()->token(),ENT_QUOTES) ?>"><input type="hidden" name="document" value="<?= htmlspecialchars($DOC->ID,ENT_QUOTES) ?>"><input type="hidden" name="version" value="<?= htmlspecialchars($DOC->VERSION,ENT_QUOTES) ?>"><button type="submit" class="btn btn-floating btn-small tooltipped waves-effect waves-light orange" data-position="bottom" data-tooltip="<?= $TXT->TooltipRestoreVersion ?>"><i class="material-icons">update</i></button></form><?php endif; ?>
            <?php else: ?>
        ...
        <?php if(MODE=="edit"): ?>
          <span class="right nowrap">
            ...
            <form method="post" action="<?= $APP->PATH ?>submit.php?act=content_delete" style="display:inline;" onsubmit="return(confirm('<?= str_replace(["'",'"'],"\'",$TXT->TooltipDeleteDocumentConfirm) ?>'))"><input type="hidden" name="token" value="<?= htmlspecialchars(Session::getInstance()->token(),ENT_QUOTES) ?>"><input type="hidden" name="document" value="<?= htmlspecialchars($DOC->ID,ENT_QUOTES) ?>"><button type="submit" class="btn btn-floating btn-small tooltipped waves-effect waves-light red" data-position="bottom" data-tooltip="<?= $TXT->TooltipDeleteDocument ?>"><i class="material-icons">delete</i></button></form>
    ...
    <?php if(!Session::getInstance()->privacyAgreeded()): ?>
      <!-- Modal Structure -->
      <div id="modal_privacy" class="modal">
        <div class="modal-content">
...
<?php if(!Session::getInstance()->privacyAgreeded()): ?>
  <script>
    $(document).ready(function(){
      $('#modal_privacy').modal({'dismissible':false,'opacity':0.72});
      $('#modal_privacy').modal('open');
    });
  </script>
<?php endif; ?>
<script>
