...
final class Document{
    ...
    private static function isDangerousUrl($url, $attrName) {
        ...
        if (preg_match('/^data\s*:/i', $url)) {
            ...
            if ($attrName === 'src' && preg_match('#^data:image/(png|jpeg|jpg|gif|webp)\s*;\s*base64,[A-Za-z0-9+/=]+$#i', $url)) {
                return false;
            }
            return true;
        }
    ...
    public function attachments():array{
        ...
        if(is_dir($this->DIR)){
            ...
            foreach($elements as $element_fe){
                ...
                $attachment->url=PATH."datasets/documents/".$this->ID."/".rawurlencode($element_fe);
                // add element to documents array
